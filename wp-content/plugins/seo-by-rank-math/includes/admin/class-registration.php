...
class Registration {
	...
	private function get_registration_url( $status ) {
		...
		if ( 'ok' === $status && $auth_data = $this->get_registration_params() ) { // phpcs:ignore
			...
			// Redirect to the Setup Wizard Agent if registration is successful.
			if ( Param::get( 'page' ) === 'rank-math-registration' ) {
				return Helper::get_admin_url( 'wizard-agent' );
			}
	...
	/**
	 * Redirect the user to the registration/connect page after plugin activation.
	 */
	public function redirect_to_welcome() {
		...
		Helper::redirect( Helper::get_admin_url( 'registration' ) );
		exit;
	}
	...
	private function can_redirect() {
		...
		if ( ( ! empty( $_GET['page'] ) && in_array( $_GET['page'], [ 'rank-math-registration', 'rank-math-wizard', 'rank-math-wizard-agent' ], true ) ) || ! current_user_can( 'manage_options' ) ) {
			return false;
		}
